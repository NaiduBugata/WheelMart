...
We chose this Problem Statement because the two-wheeler market in India is one of the biggest and fastest-growing segments. However, the online buying and selling experience is still flawed. Buyers often face...

---

### **i) Proposed Approach:**
- We will build the frontend using React to create a responsive user interface.
- We will integrate a Flask backend with a MongoDB database to manage listings, users, and dealers.
...
1. **Display:** Picture, name, and cost of EVs, bikes, and scooters.
...
1. Data flow starts with user interaction on the frontend. People browse bikes there. They apply filters or check out detailed product pages. All this happens through a React-based UI. Actions kick things off.
2. Backend handles it from there, mostly using Flask. It processes the requests, applies business logic, fetches data or updates as needed. For example, the EMI calculator uses the input to compute results.
3. Database comes next. Stores all the data (listings, users, dealers, bookings, reviews, etc.). Could be relational or NoSQL setup. Queries get optimized for fast search and filtering. Keeps the system responsive.
...
5. Finally, responses go back to the UI as processed JSON data, which gets rendered into user-friendly interfaces.
...
1. **RAG-Powered Chatbot (Text & Web):**
   - *Role:* A Retrieval-Augmented Generation framework that merges live data retrieval with deep language understanding to deliver precise and context-aware responses.
   - *Application:* Manages customer questions related to vehicle details, model comparisons, EMI calculations, upcoming releases, and general FAQs on the platform.
2. **AI Voice Assistant for Calls:**
   - *Role:* Utilizes Speech-to-Text (STT), Natural Language Processing for intent recognition, and Text-to-Speech (TTS) to automate interactions with buyers.
   - *Application:* Addresses common buyer inquiries such as pricing, availability, offers, and test ride scheduling—reducing manual workload for sellers while improving response speed.
3. **Review Analysis & Summarization:**
   - *Role:* NLP techniques scan large volumes of customer reviews, condensing them into meaningful insights and sentiment overviews.
   - *Application:* Enables buyers to quickly grasp collective feedback on vehicles and provides sellers with data-driven suggestions for service improvement.
4. **Personalized Recommendations:**
   - *Role:* Leverages a mix of collaborative filtering and content-based recommendation algorithms to predict individual preferences.
   - *Application:* Suggests best-fit vehicles, similar alternatives, and upgrade options based on browsing activity and user history.
5. **Dynamic Price Estimation:**
   - *Role:* Employs XGBoost and regression models to interpret factors such as market conditions, vehicle age, mileage, and demand trends for accurate price forecasts.
   - *Application:* Helps buyers identify fair value for used vehicles while supporting sellers in setting competitive prices.

#### **ii) Training and Evaluation Strategy**
- **Data Acquisition:** Collecting datasets from historical listings, user queries, customer reviews, and real-time market prices.
- **Data Preparation:** Managing missing fields, normalizing numerical inputs, encoding categorical data, and cleaning textual content.
- **Model Training & Validation:**
  - Pricing Models: Trained with XGBoost and regression; assessed using R² and RMSE metrics.
  - Chatbot & Call Assistant: Intent classification developed with Transformer/RNN models; validated for accuracy and response reliability.
  - Review Summarization: NLP pipelines designed to extract sentiments and key takeaways from review texts.
- **Deployment:** Models are deployed as APIs using a Flask backend, integrated seamlessly with the React frontend and connected voice platforms (e.g., Twilio).
...
- **Unit Testing:** Validates individual components such as search, filters, calculators, and AI predictions.
- **Integration Testing:** Ensures smooth interaction between frontend and backend. Confirms proper database connectivity and AI API integration.
- **System Testing:** End-to-end validation of features like browsing, comparisons, booking, AI responses, and notifications.
- **Load & Stress Testing:** Simulates peak usage to check system scalability. Verifies caching efficiency and CDN performance.
- **User Acceptance Testing (UAT):** Involves real users (e.g., dealers) to gather feedback. Focuses on usability, feature improvements, and final adjustments before deployment.
